...
export class FileClassifier {
  ...
  public classifyFiles(files: string[]): ClassifiedFile[] {
    return files.map((file) => {
      ...
      const { isFrontend, isBackend } = this.determineEnvironment(lowerPath, ext);
  ...
  private determineEnvironment(path: string, ext: string): { isFrontend: boolean; isBackend: boolean } {
    ...
    const frontendDirs = ['components', 'pages', 'app', 'views', 'ui', 'styles', 'frontend', 'client', 'public'];
    const hasFrontendDir = pathParts.some((part) => frontendDirs.includes(part));
    ...
    // Backend directories
    const backendDirs = ['controllers', 'services', 'repositories', 'middleware', 'guards', 'api', 'routes', 'server', 'backend', 'models', 'resolvers'];
    ...
    if (
      ...
      filename.includes('use') && ['.ts', '.js'].includes(ext) && hasFrontendDir
    ) {
      isFrontend = true;
    }
