...
export class SemgrepAnalyzer implements StaticAnalyzer {
  ...
  public readonly description = 'Runs Semgrep static rules to catch security issues and code quality smells';
    ...
    try {
      ...
      if (!isAvailable) {
        return {
          ...
          success: false,
          findings: [],
          error: 'Semgrep CLI is not installed or not available in system PATH.',
      ...
      return new Promise<AnalysisResult>((resolve) => {
        child_process.exec(cmd, { maxBuffer: 10 * 1024 * 1024 }, (error, stdout) => {
          ...
          try {
            ...
            for (const result of results) {
              ...
              if (context.files.length > 0 && !context.files.includes(relativeFilePath)) {
                continue;
              }

              const severity: FindingSeverity = this.mapSemgrepSeverity(result.extra?.severity || '');
              const category: FindingCategory = this.mapSemgrepCategory(result.extra?.metadata?.category || '', relativeFilePath);
              ...
              findings.push({
                ...
                standardReferences: this.mapMetadataToStandards(result.extra?.metadata || {}),
                whyItMatters: message,
                recommendation: result.extra?.metadata?.remediation || `Follow the Semgrep rule recommendation to refactor this code block and resolve the issue.`,
  ...
  private mapSemgrepCategory(semgrepCategory: string, filePath: string): FindingCategory {
