...
export class ESLintAnalyzer implements StaticAnalyzer {
  ...
  public readonly description = 'Runs ESLint with security, architecture, and accessibility checks';
  ...
  public async analyze(context: AnalysisContext): Promise<AnalysisResult> {
    ...
    try {
      ...
      for (const result of results) {
        ...
        const relativeFilePath = result.filePath.replace(context.scanDir.replace(/\\/g, '/'), '').replace(/^\//, '');

        for (const message of result.messages) {
          // Determine severity: 1 is warning, 2 is error
          const severity: FindingSeverity = message.severity === 2 ? 'high' : 'medium';

          // Classify based on rule ID
          const category = this.mapRuleIdToCategory(message.ruleId || '', relativeFilePath);

          // Map to standards
          const standardReferences = this.mapRuleIdToStandards(message.ruleId || '');
  ...
  private mapRuleIdToCategory(ruleId: string, filePath: string): FindingCategory {
    const isFrontend =
    ...
    if (ruleId.includes('security') || ruleId.includes('xss') || ruleId.includes('csrf') || ruleId.includes('eval')) {
    ...
    if (ruleId.includes('react') || ruleId.includes('next') || ruleId.includes('html')) {
      return 'accessibility'; // UX / Frontend
    }

    if (ruleId.includes('import') || ruleId.includes('module') || ruleId.includes('require')) {
  ...
  private mapRuleIdToStandards(ruleId: string): string[] {
    ...
    if (ruleId.includes('security') || ruleId.includes('xss') || ruleId.includes('eval')) {
