...
import { Finding, AIReviewResult, AIReviewResultSchema } from '@slopshield/shared';
import { AIReviewerProvider, ReviewInput } from '../interfaces/ai-reviewer-provider.interface.js';
...
export class GeminiProvider implements AIReviewerProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (apiKey) {
      ...
      this.logger.warn('GEMINI_API_KEY is not defined. AI Reviewer will operate in mock mode.');
    }
  }

  public async reviewCode(input: ReviewInput): Promise<AIReviewResult> {
    if (!this.ai) {
      this.logger.warn('Gemini AI client not initialized (missing API key). Returning mock review.');
    ...
    while (retries >= 0) {
      try {
        ...
        this.logger.warn(`Gemini review failed. Retries remaining: ${retries}. Error: ${err.message || err}`);
        retries--;
        if (retries < 0) {
          throw new Error(`AI review failed after multiple attempts: ${err.message || err}`);
        }
      }
    }
  ...
  public async generateFixPlan(findings: Finding[], codeContext: string): Promise<string[]> {
    if (!this.ai) {
      return ['Verify environment variables.', 'Mock plan: resolve findings manually.'];
    }

    const findingsDesc = findings
      .map((f, i) => `${i + 1}. [${f.severity}] ${f.title} in ${f.file}${f.line ? `:${f.line}` : ''}\nWhy: ${f.whyItMatters}\nRec: ${f.recommendation}`)
      .join('\n\n');
    ...
    try {
      const response = await this.ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: 'You are a Senior Principal Engineer. Provide a concise, step-by-step technical fix plan. Output ONLY a valid JSON string array.',
          responseMimeType: 'application/json',
        },
      });
      ...
      this.logger.error(`Failed to generate fix plan via Gemini: ${err.message}`);
    }

    return ['Resolve security findings first.', 'Implement missing validation schemas.', 'Update test files to verify changes.'];
  }
  ...
  public async summarizeForLark(scanReport: any): Promise<string> {
    ...
    try {
      const response = await this.ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: 'You are a technical product manager. Provide a single, extremely punchy, direct summary of the scan results. Focus on blocking or critical items. Do not exceed 3 sentences.',
        },
      });
  ...
  private getMockReviewResult(input: ReviewInput): AIReviewResult {
    return {
      summary: 'Mock analysis report. Set GEMINI_API_KEY environment variable to enable full AI-assisted scan features.',
      findings: [],
      recommended_tests: ['Add unit tests for all updated modules.'],
      refactor_plan: ['Configure GEMINI_API_KEY in .env file.', 'Rerun scan to trigger AI review analysis.'],
    };
  }
}
