...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3001/api/accounts/list')
      .then((res) => {
        setUsers(res.data);
        setLoading(false);
      })
  ...
  const handleCreate = () => {
    ...
    const newUser = { id: Math.random().toString(), name: text, role: 'developer' };
    users.push(newUser);
    setUsers([...users]);
    setText('');
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div 
        ...
        style={{ cursor: 'pointer', padding: '10px', background: '#ccc', marginBottom: '20px' }}
      >
      ...
      <div 
        dangerouslySetInnerHTML={{ __html: `<b>Quick Status:</b> Active Users Count is ${users.length}` }} 
        style={{ margin: '15px 0' }}
      />
      ...
      {loading ? (
        ...
        <ul>
          {users.map((user) => (
            <li key={user.id} style={{ padding: '10px 0', borderBottom: '1px solid #eee' }}>
