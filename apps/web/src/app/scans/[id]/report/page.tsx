...
import { 
  ArrowLeft, ShieldAlert, Sparkles, CheckSquare, RefreshCw, Download, 
  ExternalLink, FileWarning, ShieldCheck, HelpCircle, Loader2 
} from 'lucide-react';
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4">
        ...
        <p className="text-sm font-mono text-gray-500">Compiling quality report data...</p>
      </div>
    );
  }
  ...
  if (isError || !scan) {
    return (
      <div className="max-w-md mx-auto py-24 text-center space-y-4">
        ...
        <p className="text-xs text-gray-500">Verify network connection or DB schema initialization.</p>
      </div>
    );
  }

  const findings = scan.findings || [];
  const blockingFindings = findings.filter((f: any) => f.blocking && !f.falsePositive);
    ...
    <div className="space-y-8 relative">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between border-b border-border pb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          >
            <RefreshCw className={`w-4 h-4 ${actionLoading === 'rerun' ? 'animate-spin' : ''}`} />
            Rerun Audit
          </button>
        </div>
      </div>
      ...
      {blockingFindings.length > 0 && (
        <div className="p-4 rounded-sm bg-destructive/10 border border-destructive/20 flex gap-3.5 items-start text-destructive">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-sm">Merge Blocked by Quality Rules</h4>
            <p className="text-xs leading-relaxed opacity-90">
              This codebase has been automatically blocked from production release. We detected <strong>{blockingFindings.length} unresolved critical blockers</strong>. Rerun the scan once the code has been patched.
            </p>
          </div>
        </div>
      )}

      {blockingFindings.length === 0 && scan.overallScore >= (scan.project?.minimumScore || 80) && (
        <div className="p-4 rounded-sm bg-status-passed/10 border border-status-passed/20 flex gap-3.5 items-start text-status-passed">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs leading-relaxed opacity-90">
              The scan meets all quality requirements. Overall quality score ({scan.overallScore}/100) is at or above the threshold ({scan.project?.minimumScore || 80}/100) with zero active blockers.
            </p>
          </div>
        </div>
      )}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 border border-border bg-card p-6 rounded-sm flex flex-col justify-between">
          ...
          <div className="border-t border-border pt-4 mt-6 grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Findings</p>
              <p className="font-display text-2xl font-extrabold text-foreground mt-1">{activeFindings.length}</p>
            </div>
            <div>
              <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Blockers</p>
              <p className="font-display text-2xl font-extrabold text-destructive mt-1">{blockingFindings.length}</p>
            </div>
            <div>
              <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Profile Mode</p>
              <p className="text-xs font-bold text-ring mt-2.5 uppercase tracking-wider">{scan.scanMode}</p>
            </div>
          </div>
        </div>
      ...
      {selectedFindingId && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-end transition-opacity">
          {/* Backdrop Closer */}
          <div className="absolute inset-0" onClick={() => setSelectedFindingId(null)} />
