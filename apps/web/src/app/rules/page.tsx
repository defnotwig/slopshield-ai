...
import { Search, RefreshCw, AlertTriangle, ShieldCheck, HelpCircle } from 'lucide-react';
...
export default function RulesLibraryPage() {
  ...
  const filteredRules = rules.filter((rule) => {
    const matchesCategory =
      filterCategory === 'all' || rule.category.toLowerCase().includes(filterCategory);
    const matchesSearch =
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Quality Rules Library</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-white dark:bg-gray-900/30 p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
        ...
        <div className="flex flex-col w-full sm:w-auto">
          <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-1">Category</span>
          <select
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredRules.map((rule) => {
            const appliesToArray = Array.isArray(rule.applies_to) ? rule.applies_to : [];
            const standardsArray = Array.isArray(rule.standards) ? rule.standards : [];
              ...
              >
                <div>
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="space-y-1">
                      ...
                      <h4 className="font-bold text-sm text-gray-800 dark:text-gray-100">{rule.title}</h4>
                    </div>
                ...
                <div className="border-t border-gray-200 dark:border-gray-800 pt-3 flex items-center justify-between gap-4 text-[10px] font-mono text-gray-500">
                  <div className="truncate max-w-[150px]">
                    <span className="font-bold">Applies:</span> {appliesToArray.join(', ') || 'Any'}
                  </div>
                  <div className="truncate max-w-[150px] text-right">
                    <span className="font-bold">Refs:</span> {standardsArray.join(', ') || 'N/A'}
                  </div>
                </div>
              </div>
            );
