...
export function SeverityBadge({ severity }: SeverityBadgeProps) {
  ...
  const styles: Record<string, string> = {
    critical:
      "bg-red-500/10 text-red-500 border border-red-500/20 dark:bg-severity-critical/10 dark:text-severity-critical dark:border-severity-critical/20",
    ...
    medium:
      "bg-yellow-500/10 text-yellow-600 border border-yellow-500/20 dark:bg-severity-medium/10 dark:text-severity-medium dark:border-severity-medium/20",
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold uppercase tracking-wider ${currentStyle}`}
    >
      {severity}
    </span>
  );
